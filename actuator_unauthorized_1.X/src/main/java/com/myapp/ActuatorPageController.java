...
public class ActuatorPageController {
    ...
    @GetMapping(value = {"/", "/actuator-unauthorized-1x"}, produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String index() {
        return "<!DOCTYPE html>" +
                "<html lang=\"zh-CN\"><head><meta charset=\"UTF-8\"><title>Actuator 1.X 未授权测试页</title>" +
                style() +
                "</head><body><div class=\"container\"><h1>Actuator 1.X 未授权测试页</h1>" +
                "<p class=\"hint\">点击按钮会直接访问当前服务的 <code>/trace</code> 接口。你也可以修改目标路径后再发起请求。</p>" +
                "<label for=\"path\">目标路径</label><input id=\"path\" value=\"/trace\" />" +
                "<div class=\"actions\"><button class=\"attack\" onclick=\"fillTrace()\">填充 /trace</button><button class=\"send\" onclick=\"sendCurrent()\">发送当前请求</button></div>" +
